...
namespace Nop.Web.Controllers
{
    public partial class CommonController : BasePublicController
    {
        ...
        {
            var email = model.ActgEmail.Trim();
            var fullName = model.FirstName;
            string subject = string.Format(_localizationService.GetResource("ContactUs.EmailSubject"),
                _storeContext.CurrentStore.GetLocalized(x => x.Name));
        ...
        {
            ViewBag.WorkingThemeName = _themeContext.WorkingThemeName;
            ViewBag.IsAboutUsPageAvailable = (_topicService.GetTopicBySystemName("AboutUs", _storeContext.CurrentStore.Id) != null);

            return View();
        }

        public ActionResult NowCommerceSignUp()
        {
            var model = new NowCommerceSignUpModel();
            model = _commonModelFactory.PrepareNowCommerceSignUpModel(model);
            return View(model);
        }

        [HttpPost, ActionName("NowCommerceSignUp")]
        [PublicAntiForgery]
        [CaptchaValidator]
        public ActionResult NowCommerceSignUpSend(NowCommerceSignUpModel model, bool captchaValid)
        {
            model = _commonModelFactory.PrepareNowCommerceSignUpModel(model);
            ...
            {
                _workflowMessageService.SendNowCommerceSignUpMessage(_workContext.WorkingLanguage.Id, model.DealerName, model.ContactPerson, model.ContactEmail, model.AdditionalComments);

                model.SuccessfullySent = true;
                model.Result = _localizationService.GetResource("NowCommerceSignUp.SuccessfullySent");
            }
            return View(model);
        }
