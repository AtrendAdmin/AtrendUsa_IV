...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual string[] GetStores(Product product)
        {
            if (product.LimitedToStores)
            {
                return _storeMappingService.GetStoreMappings<Product>(product).Select(s => s.Store.Name).ToArray();
            }
            return new string[] { };
        }
        ...
        public virtual byte[] ExportProductsToXlsx(IEnumerable<Product> products)
        {
            var properties = new[]
            {
                ...
                new PropertyByName<Product>("Stores", p => string.Join(";", GetStores(p)))
            };
