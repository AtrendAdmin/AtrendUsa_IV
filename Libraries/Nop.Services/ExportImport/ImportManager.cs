        ...
        public virtual void ImportProductsFromXlsx(Stream stream)
        {
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                for (var iRow = 2; iRow < endRow; iRow++)
                {
                    ...
                    //update stores
                    var storesProperty = manager.GetProperty("Stores");

                    string[] actualStoreNames = _storeMappingService.GetStoreMappings<Product>(product).Select(s => s.Store.Name).ToArray();
                    string[] receivedStoreNames = storesProperty.StringValue.Split(new[] { ';' }, StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).ToArray();

                    if(!actualStoreNames.SequenceEqual(receivedStoreNames))
                    {
                        //delete all existing mappings for this product
                        var existingStoreMappings = _storeMappingService.GetStoreMappings<Product>(product).ToList();
                        ...
                        //add new mappings
                        foreach (var storeName in receivedStoreNames)
                        {
                            var store = allStores.Where(s => s.Name == storeName).FirstOrDefault();
