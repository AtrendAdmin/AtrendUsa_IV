...
namespace Nop.Services.Messages
{
    public partial class WorkflowMessageService : IWorkflowMessageService
    {
        ...
        /// <summary>
        /// Sends "NowCommerce" sign up message
        /// </summary>
        ...
        /// <returns></returns>
        public virtual int SendNowCommerceSignUpMessage(int languageId, string dealerName, string contactPerson,
            string contactEmail, string additionalComments)
        {
            var store = _storeContext.CurrentStore;
            
            var messageTemplate = GetActiveMessageTemplate(MessageTemplateSystemNames.NowCommerceSignUpMessage, store.Id);
            if (messageTemplate == null)
                return 0;

            string subject = $"{store.Name}. Now Commerce account sign up request";
            string fromEmail = contactEmail;
            string fromName = contactPerson;

            var tokens = new List<Token>();
            tokens.Add(new Token("NowCommerceSignUp.StoreName", store.Name));
            tokens.Add(new Token("NowCommerceSignUp.DealerName", dealerName));
            tokens.Add(new Token("NowCommerceSignUp.ContactPerson", contactPerson));
            tokens.Add(new Token("NowCommerceSignUp.ContactEmail", contactEmail));
            tokens.Add(new Token("NowCommerceSignUp.AdditionalComments", additionalComments));

            var emailAccount = GetEmailAccountOfMessageTemplate(messageTemplate, languageId);

            return SendNotification(messageTemplate, emailAccount, languageId, tokens, emailAccount.Email,
